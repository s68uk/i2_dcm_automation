...
"""
PACKAGE: 2
Last updated: 27 Aug 2026 — restructured against the full initial
requirement: report is now organized into named categories (Distress,
Named Deals, Law Firm News, Adviser Activity, Competitor Moves, Clearing
Systems, M&A Escrow, Sanctions), each item carries a "why it matters to
i2" line, empty categories say so explicitly rather than being omitted,
and a new M&A holdback/escrow category was added (Lloyds, HSBC, BNY,
Citibank, Ebury Partners, Corpay, Alpha FX + escrow/holdback/earn-out
terms, gated so the bank names alone can't trigger it).
...
def score_story(story, kw):
    ...
    if is_self_referential:
        category = "firm_intel"
        why_it_matters = "Firm/adviser/competitor's own personnel or ownership news — competitive intel, not a client situation"
    elif named_hits:
        category = "named_deals"
        why_it_matters = f"Named situation i2 already tracks ({', '.join(named_hits)})"
    elif escrow_term_hits:
        category = "ma_escrow"
        entities_note = f" involving {', '.join(escrow_entity_hits)}" if escrow_entity_hits else ""
        why_it_matters = f"M&A holdback/escrow relationship opportunity{entities_note}"
    elif matched_competitor and any_core_signal:
        category = "competitor_moves"
        why_it_matters = f"Competitor activity — {', '.join(matched_competitor)}"
    elif firm_hits and any_core_signal:
        category = "law_firm_news"
        why_it_matters = f"Law-firm restructuring mandate / relationship opportunity — {', '.join(firm_hits)}"
    elif matched_other_adviser and any_core_signal:
        category = "adviser_activity"
        why_it_matters = f"Financial adviser activity on a restructuring mandate — {', '.join(matched_other_adviser)}"
    elif matched_clearing:
        category = "clearing_systems"
        why_it_matters = f"Clearing/settlement system policy or procedure change — {', '.join(matched_clearing)}"
    elif sanctions_hits:
        category = "sanctions"
        why_it_matters = "Sanctions / cross-border risk relevant to distressed or cross-border debt"
    elif any_core_signal:
        category = "distress_general"
        why_it_matters = "Distressed-debt / restructuring situation"
