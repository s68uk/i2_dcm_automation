...
"""
PACKAGE: 4
Last updated: 27 Aug 2026 — "Why it matters to i2" moved to appear once
under each category heading (e.g. "Competitor Moves (3 items) / Why it
matters to i2: ...") instead of being repeated on every individual item.
...
def build_briefing(scored_deduped):
    ...
    for key, heading, why in REPORT_CATEGORIES:
        ...
        lines.append(f"Why it matters to i2: {why}")
        lines.append("-" * 80)
        if not items:
    ...
    lines.append(f"Why it matters to i2: {FIRM_INTEL_WHY}")
    lines.append("-" * 80)
    if not firm_intel:
        lines.append("Nothing relevant in this cycle.")
    for s in firm_intel:
