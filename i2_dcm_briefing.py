...
"""
PACKAGE: 1
Last updated: 27 Aug 2026 — removed "Option A" label from the briefing
report header (report now just reads "Distressed-debt & restructuring").
...
def build_briefing(scored_deduped):
    ...
    lines.append(f"Distressed-debt & restructuring ({len(option_a)} items)")
    lines.append("-" * 80)
    if not option_a:
