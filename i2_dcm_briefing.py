...
"""
PACKAGE: 6
Last updated: 28 Aug 2026 — added cross-run memory (seen_stories.json).
Found via live testing: several sources (GLAS, Milbank, etc.) only
publish a few times a week, so without tracking what was already
reported, the same items would repeat verbatim in the briefing for days
until they scrolled off each source's own short listing window. Now the
briefing only shows items NEW since the last run, with a clear note at
the top when there's nothing new anywhere (distinct from a failure).
...
SEEN_PATH = BASE_DIR / "seen_stories.json"
SEEN_RETENTION_DAYS = 90  # prune entries older than this so the file doesn't grow forever
...
# ---------------------------------------------------------------------------
# Cross-run memory — a daily briefing should show what's NEW since last run,
# not re-report the same items every day until they scroll off a source's
# own short listing window. Found via live testing 28 Aug 2026: several
# sources (GLAS, Milbank, etc.) only publish a few times a week, so without
# this, the same items would repeat verbatim for days.
# ---------------------------------------------------------------------------
def load_seen_links():
    ...
    try:
        ...
        log.warning("Could not read %s (%s) — starting with empty seen-store", SEEN_PATH, e)
        return {}
...
def save_seen_links(seen):
    try:
        ...
        log.warning("Could not write %s: %s", SEEN_PATH, e)


def filter_new_and_update_seen(scored_deduped, seen):
    """Drop items whose link was already reported in a previous run; mark
    everything shown this run as seen. Prunes entries older than
    SEEN_RETENTION_DAYS so the file doesn't grow forever."""
    today_str = datetime.now().strftime("%Y-%m-%d")
    today = datetime.now()
    new_items = []
    for s in scored_deduped:
        if s.get("category") == "excluded":
            continue  # never tracked — true noise, not something to remember
        link = s.get("link")
        if link in seen:
            continue
        new_items.append(s)
    ...
    for link, seen_date in seen.items():
        try:
            age_days = (today - datetime.strptime(seen_date, "%Y-%m-%d")).days
        except Exception:
            age_days = 0  # malformed entry — keep it rather than crash
        if age_days <= SEEN_RETENTION_DAYS:
            pruned[link] = seen_date

    return new_items, pruned
...
# (category_key, printed heading, one-line "why it matters" for the whole
# category — shown once under the heading, not repeated per item)
REPORT_CATEGORIES = [
    ("distress_general", "Distressed-Debt & Restructuring Situations",
     "Core distress signal — default, covenant breach, Chapter 11, scheme of arrangement, etc."),
    ("named_deals", "Named Deals & Situations i2 Already Touches",
     "i2 already has a direct relationship or exposure on these names."),
    ("law_firm_news", "Law Firm Restructuring Practice News",
     "Relationship / referral channel — mandates and moves at firms i2 works with."),
    ("adviser_activity", "Financial Adviser Activity",
     "Relationship / referral channel — restructuring mandates at advisers i2 works with."),
    ("competitor_moves", "Competitor Moves",
     "Kroll/GLAS activity — direct competitor intelligence."),
    ("clearing_systems", "Clearing System Changes",
     "Euroclear/Clearstream/DTC policy or procedure changes affecting settlement/accreditation."),
    ("ma_escrow", "M&A Holdback / Escrow Activity",
     "Potential escrow/holdback agency relationship opportunity."),
    ("sanctions", "Sanctions / OFAC",
     "Sanctions or cross-border risk relevant to distressed/cross-border debt."),
]
...
FIRM_INTEL_WHY = "Personnel or ownership news at a firm/adviser/competitor — not a client situation."


def build_briefing(scored_deduped):
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    by_category = {key: [] for key, _, _ in REPORT_CATEGORIES}
    firm_intel = []
    for s in scored_deduped:
        cat = s.get("category")
    ...
    if not scored_deduped:
        lines.append("\nNo NEW items since the last run across any category. All sources "
                      "fetched successfully — this means nothing has changed on any "
                      "watched page since last time, not that anything failed.")

    for key, heading, why in REPORT_CATEGORIES:
    ...
    lines.append(f"Relevance: {FIRM_INTEL_WHY}")
    lines.append("-" * 80)
    if not firm_intel:
        lines.append("Nothing relevant in this cycle.")
    for s in firm_intel:
...
def main():
    ...
    try:
        ...
        new_items, seen = filter_new_and_update_seen(deduped, seen)
        save_seen_links(seen)
        log.info("%d relevant items total, %d are new since last run", len(deduped), len(new_items))

        briefing = build_briefing(new_items)
